    ...
    if request.method == 'POST':
        ...
        if 'Approve' in request.form:
            ...
            if not account:
                ...
                #send email depeding if it is an appeal or first time application 
...
def appeal():
    ...
    if request.method == "POST":
        ...
        if account or applied:
            msg = "You are already in the system - please check your email for a message from Whiteboard or try a different username"
    ...
    if 'loggedin' in session:
        if request.method == "POST":
            ...
            if 'Approve' in request.form:
                ...
                cursor.execute('INSERT INTO tb_group_members (group_id, user_name) VALUES (%s, %s)', (group_id, [session['username']]))
                ...
                cursor.execute('DELETE from tb_invite WHERE user_id = %s AND group_id = %s', ([session['user_id']],group_id))
                mysql.connection.commit()

            #if user reject an invite 
            elif 'Reject' in request.form: 
                rejection = request.form['rejection']
                message = "Thank you for the consideration, but I will not accept invitation to this group. I cannot join this group, " + rejection
                group_id = request.form['group_id']
                ...
                if not user_name_exist:
                    ...
                    cursor.execute('SELECT * FROM tb_whitelist WHERE user_id = %s AND user_name_friend = %s', ([session['user_id']],user_whitelist))
                    exist = cursor.fetchone()
                    cursor.execute('SELECT * FROM tb_user_blacklist WHERE user_id = %s AND user_name_blocked = %s', ([session['user_id']],user_whitelist))
                    otherlist = cursor.fetchone()
                    #if not then insert it into the correct list 
                    if not exist and not otherlist:
                        cursor.execute("INSERT INTO tb_whitelist (user_id, user_name_friend)" "VALUES (%s,%s)", ([session['user_id']],user_whitelist)) 
                        mysql.connection.commit()
            ...
            #user adding into the black list 
                ...
                if not user_name_exist:
                    ...
                    cursor.execute('SELECT * FROM tb_user_blacklist WHERE user_id = %s AND user_name_blocked = %s', ([session['user_id']],user_blacklist))
                    exist = cursor.fetchone()
                    cursor.execute('SELECT * FROM tb_whitelist WHERE user_id = %s AND user_name_friend = %s', ([session['user_id']],user_blacklist))
                    otherlist = cursor.fetchone()
                    #if not put them into the list 
                    if not exist and not otherlist:
                        cursor.execute("INSERT INTO tb_user_blacklist (user_id, user_name_blocked)" "VALUES (%s,%s)", ([session['user_id']],user_blacklist)) 
                        mysql.connection.commit()  
        ...
        cursor.execute('SELECT tb_invite.*, tb_group.group_name, tb_group.group_describe FROM tb_group INNER JOIN tb_invite ON tb_invite.group_id = tb_group.group_id WHERE tb_invite.user_id = %s', [session['user_id']])
        ...
        return render_template('profile.html', account=account, post_history=post_history, group_info=group_info, invitation=invitation, friends=friends, blocked=blocked)
...
def invite(group_id):
    if request.method == 'POST':
        ...
        #adder is in the blacklist display automatic rejection message in group 
            ...
            cursor.execute('INSERT INTO tb_chat (user_id, group_id, chat_content) VALUES (%s, %s, %s)',
                    ([account['user_id']], group_id, "Thank you for the consideration, but I will not accept invitation to this group."))
            mysql.connection.commit()

        #adder is in neither, send a invitation to the invitee's profile page   
        else: 
            cursor.execute("SELECT * FROM tb_invite WHERE user_id =%s AND group_id = %s", (account['user_id'],group_id))
            invited = cursor.fetchone()
            #check to see if invitations already sent
            if invited:
                flash('This users is already invited')
            else:
                cursor.execute("INSERT INTO tb_invite (user_id,group_id)" " VALUES (%s,%s)", (account['user_id'],group_id))
                mysql.connection.commit()
        return redirect(url_for('into_group', group_id=group_id))
